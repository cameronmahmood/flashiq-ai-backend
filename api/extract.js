...
// Supports: PDF, PPTX (read slides' XML), images (OCR via OpenAI Vision), TXT.
// Env: OPENAI_API_KEY
...
function extractTextFromPptx(buffer) {
  ...
  for (const e of slideEntries) {
    ...
    if (slideText.trim()) chunks.push(slideText);
  }
  return chunks.join("\n\n");
}
  ...
  const resp = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "user",
        content: [
          { type: "text", text: "Extract legible text from this image. If handwritten, transcribe best-effort." },
...
const isText = (mt, name) => mt?.startsWith?.("text/") || /\.txt$/i.test(name);
  ...
  try {
    ...
    for (const f of files) {
      ...
      try {
        ...
        if (!text.trim() && isImage(f.mimeType)) {
        ...
        results.push(text.trim()
          ? { file: f.filename, ok: true, text }
          : { file: f.filename, ok: false, error: "No extractable text found" });
      } catch (err) {
        console.error("EXTRACT_ERR:", f?.filename, err);
    ...
    console.error("EXTRACT_TOP_ERR:", e);
